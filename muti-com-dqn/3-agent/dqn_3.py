...
from ANN_3 import createQNetwork_Custom, copyTargetQNetwork, createQNetwork1
...
def main():
    ...
    if testing:
        ...
        eps_start = 0.11
        epoch_start = 19
        param_file1a = "saved_networks_agent1/network-dqn_mx0019.params"
        param_file2a = "saved_networks_agent2/network-dqn_mx0019.params"
        param_file3a = "saved_networks_agent2/network-dqn_mx0019.params"

    if not testing:
        logging_config(folder_name="log", name="com_network_a3")
    ...
    replay_memory3 = ReplayMemory(state_dim=(78,),
                                  history_length=history_length,
                                  memory_size=replay_memory_size,
                                  replay_start_size=replay_start_size, state_dtype='float16')
    ...
    a1_target1 = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1)
    a1_target32 = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32)
    a1_Qnet = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32)
    ...
    a3_target1 = createQNetwork1(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1)
    a3_target32 = createQNetwork1(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32)
    a3_Qnet = createQNetwork1(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32)
    ...
    if param_file2a is not None:
        a3_Qnet.load_params(param_file3a)
    copyTargetQNetwork(a3_Qnet, a3_target1)
    copyTargetQNetwork(a3_Qnet, a3_target32)

    print_params(a1_Qnet)
    print_params(a2_Qnet)
    print_params(a3_Qnet)
    ...
    for epoch in range(epoch_start, epoch_num):
            ...
            while not env.game_over():
                if replay_memory1.size >= history_length and replay_memory1.size > replay_start_size:
                    ...
                    if do_exploration:
                        ...
                        # signal
                        signal1 = action_matrix[action1]
                        current_state2 = np.append(current_state2[:, :-4], [signal1], axis=1)
                        ...
                        # signal
                        signal2 = action_matrix[action2]
                        current_state3 = np.append(current_state3[:, :-4], [signal2], axis=1)
                        state3 = nd.array(current_state3.reshape((1,) + current_state3.shape),
                                          ctx=q_ctx)
                        a3_target1.forward(mx.io.DataBatch([state3], []))
                        q_value3 = a3_target1.get_outputs()[0].asnumpy()[0]
                        action3 = numpy.argmax(q_value3)
                        episode_q_value += q_value2[action3]
                ...
                replay_memory2.append(np.append(next_ob[1].flatten(), action_matrix[action1]), action2, reward[1],
                                      terminal_flag)
                replay_memory3.append(np.append(next_ob[2].flatten(), action_matrix[action2]), action3, reward[2],
                                      terminal_flag)
                ...
                if total_steps % update_interval == 0 and replay_memory1.size > replay_start_size:
                    ...
                    state_batch3, actions3, rewards3, nextstate_batch3, terminate_flags3 = replay_memory3.sample(
                        batch_size=minibatch_size)
                    state_batch3 = nd.array(state_batch3, ctx=q_ctx)
                    actions_batch3 = nd.array(actions3, ctx=q_ctx)
                    reward_batch3 = nd.array(rewards3, ctx=q_ctx)
                    terminate_flags3 = nd.array(terminate_flags3, ctx=q_ctx)
                    ...
                    a3_target32.forward(mx.io.DataBatch([nd.array(nextstate_batch3, ctx=q_ctx)], []))
                    Qvalue3 = a3_target32.get_outputs()[0]
                    ...
                    y_batch3 = reward_batch3 + nd.choose_element_0index(Qvalue3, nd.argmax_channel(Qvalue3)) * (
                        1.0 - terminate_flags3) * discount

                    a1_Qnet.forward(mx.io.DataBatch([state_batch1, actions_batch1, y_batch1], []), is_train=True)
                    a1_Qnet.backward()
                    a1_Qnet.update()

                    a2_Qnet.forward(mx.io.DataBatch([state_batch2, actions_batch2, y_batch2], []), is_train=True)
                    a2_Qnet.backward()
                    a2_Qnet.update()

                    a3_Qnet.forward(mx.io.DataBatch([state_batch3, actions_batch3, y_batch3], []), is_train=True)
                    a3_Qnet.backward()
                    a3_Qnet.update()
                    ...
                    if training_steps % 10 == 0:
                        ...
                        loss3 = 0.5 * nd.square(
                            nd.choose_element_0index(a3_Qnet.get_outputs()[0], actions_batch3) - y_batch3)
        ...
        a3_Qnet.save_params('saved_networks_agent3/network-dqn_mx%04d.params' % epoch)
