    ...
    for epoch in range(epoch_num):
        ...
        while steps_left > 0:
            ...
            while not all_done:
                ...
                data_batch1 = mx.io.DataBatch(data=[mx.nd.array(step_ob, ctx=q_ctx)], label=None)
                ...
                Qnet.model.reshape([('data', (num_envs, 78))])
                ...
                for i, env in enumerate(envs):
                    if not done[i]:
                        next_ob, reward, done[i] = env.act_action(
                            [action_map1[step_action1[i]]])
                        env_ob[i].append(step_ob[i])
                        env_action[i].append(step_action1[i])
                ...
                if t == t_max:
                    ...
                    for i in xrange(len(env_value)):
                        if env_value[i].__len__() != 51:
                            print len(env_value[i])
                            print t
                            print env_value[i]
                        R = env_value[i][-1]
                        adv_R = []
                        for j in range(len(env_reward[i]))[::-1]:
                            ...
                            adv_R.append(R - env_value[i][j])
                        advs_Q.extend(adv_R[::-1])
