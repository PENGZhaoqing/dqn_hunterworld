...
def main():
    ...
    if not os.path.exists('a1_actQnet'):
        os.makedirs('a1_actQnet')
    if not os.path.exists('a1_comNet'):
        os.makedirs('a1_comNet')
    if not os.path.exists('a2_actQnet'):
        os.makedirs('a2_actQnet')
    ...
    if testing:
        ...
        a1_act_param_file = 'a1_actQnet/network-dqn_mx0011.params'
        a1_com_param_file = 'a1_comNet/network-dqn_mx0011.params'
        a2_act_param_file = 'a2_actQnet/network-dqn_mx0011.params'
        a2_com_param_file = 'a2_comNet/network-dqn_mx0011.params'
        epoch_start = 19
        eps_start = 0.1
        eps_min = 0.1
            ...
            while not env.game_over():
                if replay_memory1.size >= history_length and replay_memory1.size > replay_start_size:
                    ...
                    if do_exploration:
                        ...
                        signal1[np.random.randint(signal1.shape[0])] = 1
                        signal2 = np.zeros(signal_num)
                        signal2[np.random.randint(signal2.shape[0])] = 1
                    else:
                        ...
                        signal1 = a2_comTarget.forward_one(nd.array(current_state1[:, 1], ctx=q_ctx))
                        signal2 = a1_comTarget.forward_one(nd.array(current_state2[:, 0], ctx=q_ctx))
                        q_value1 = a1_actTarget.forward_one(state1, signal1)[0].asnumpy()
                        q_value2 = a1_actTarget.forward_one(state2, signal2)[0].asnumpy()
                        signal1 = signal1[0].asnumpy()
                        signal2 = signal2[0].asnumpy()
                ...
                replay_memory1.append_signal(next_ob, signal2, action1, reward[0], terminal_flag)
                replay_memory2.append_signal(next_ob, signal1, action2, reward[1], terminal_flag)
                ...
                if total_steps % update_interval == 0 and replay_memory1.size > replay_start_size:
                    ...
                    state_batch1, signal1, actions1, rewards1, nextstate_batch1, next_signal1, terminate_flags1 = replay_memory1.sample_one(
                        batch_size=minibatch_size)
                    state_batch2, signal2, actions2, rewards2, nextstate_batch2, next_signal2, terminate_flags2 = replay_memory2.sample_one(
                        batch_size=minibatch_size)

                    signal1 = nd.array(signal1, ctx=q_ctx)
                    next_signal1 = nd.array(next_signal1, ctx=q_ctx)
                    signal2 = nd.array(signal2, ctx=q_ctx)
                    next_signal2 = nd.array(next_signal2, ctx=q_ctx)

                    a1_state_batch = nd.array(state_batch1[:, 0], ctx=q_ctx)
                    a1_signal_state = nd.array(state_batch1[:, 1], ctx=q_ctx)
                    a2_state_batch = nd.array(state_batch2[:, 1], ctx=q_ctx)
                    a2_signal_state = nd.array(state_batch2[:, 0], ctx=q_ctx)

                    a1_nextstate_batch = nd.array(nextstate_batch1[:, 0], ctx=q_ctx)
                    # a1_signal_nextstate = nd.array(nextstate_batch1[:, 1], ctx=q_ctx)
                    a2_nextstate_batch = nd.array(nextstate_batch2[:, 1], ctx=q_ctx)
                    # a2_signal_nextstate = nd.array(nextstate_batch2[:, 0], ctx=q_ctx)
                    ...
                    a1_Qvalue = a1_actTarget.forward_batch(a1_nextstate_batch, next_signal1)
                    a2_Qvalue = a2_actTarget.forward_batch(a2_nextstate_batch, next_signal2)
                    ...
                    Qnet1 = a1_actQnet.forward_backward(a1_state_batch, signal1, actions_batch1, y_batch1)
                    Qnet2 = a2_actQnet.forward_backward(a2_state_batch, signal2, actions_batch2, y_batch2)

                    a1_comNet.forward_backward(a2_signal_state, signal2)
                    a2_comNet.forward_backward(a1_signal_state, signal1)
