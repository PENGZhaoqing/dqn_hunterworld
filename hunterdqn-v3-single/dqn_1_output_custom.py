...
from ANN_1 import createQNetwork_Custom, copyTargetQNetwork
...
def main():
    ...
    replay_memory = ReplayMemory(state_dim=(218,),
    ...
    target1 = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1)
    target32 = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32)
    Qnet = createQNetwork_Custom(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32)
            ...
            while not env.game_over():
                ...
                if total_steps % update_interval == 0 and replay_memory.size > replay_start_size:
                    ...
                    if training_steps % 10 == 0:
                        loss = 0.5 * nd.square(nd.choose_element_0index(Qnet.get_outputs()[0], actions_batch) - y_batch)
                        episode_loss += nd.sum(loss).asnumpy()
