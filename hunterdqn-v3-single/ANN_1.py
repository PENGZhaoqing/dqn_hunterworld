...
def sym1(actions_num, predict=False):
    data = mx.symbol.Variable('data')
    data = mx.symbol.Flatten(data=data)
    fc1 = mx.symbol.FullyConnected(data=data, name='fc1', num_hidden=528)
...
def createQNetwork(actions_num, q_ctx, bef_args=None, isTrain=True, batch_size=32):
    if isTrain:
        modQ = mx.mod.Module(symbol=sym(actions_num), data_names=('data', 'actionInput',), label_names=("yInput",),
                             context=q_ctx)
        modQ.bind(data_shapes=[('data', (batch_size, 1, 74)), ('actionInput', (batch_size, actions_num))],
                  label_shapes=[('yInput', (batch_size,))],
                  for_training=True)
        ...
        modQ = mx.mod.Module(symbol=sym(actions_num, predict=True), data_names=('data',), label_names=None,
                             context=q_ctx)
        modQ.bind(data_shapes=[('data', (batch_size, 1, 74))],
                  for_training=False)
        modQ.init_params(initializer=mx.init.Xavier(factor_type="in", magnitude=2.34), arg_params=bef_args)
...
def createQNetwork_Custom(actions_num, q_ctx, bef_args=None, isTrain=True, batch_size=32):
    if isTrain:
        modQ = mx.module.Module(symbol=sym1(actions_num), data_names=('data', 'dqn_action', "dqn_target"),
                                label_names=None,
                                context=q_ctx)
        bind(modQ,
             data_shapes=[('data', (batch_size, 1, 218)), ('dqn_action', (batch_size,)), ('dqn_target', (batch_size,))],
             for_training=True)
        modQ.init_params(initializer=mx.init.Xavier(factor_type="in", magnitude=2.34), arg_params=bef_args)
        modQ.init_optimizer(
            optimizer='adam',
        ...
        modQ = mx.module.Module(symbol=sym1(actions_num, predict=True), data_names=('data',), label_names=None,
                                context=q_ctx)
        bind(modQ, data_shapes=[('data', (batch_size, 1, 218))],
             for_training=False)
        modQ.init_params(initializer=mx.init.Xavier(factor_type="in", magnitude=2.34), arg_params=bef_args)
