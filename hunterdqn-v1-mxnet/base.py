...
class Base(object):
    ...
    def __init__(self, data_shapes, sym_gen, params=None, aux_states=None,
                 ...
                 initializer=mx.init.Xavier(factor_type="in", rnd_type="gaussian", magnitude=2.34),
                 ctx=mx.gpu(), name='Net'):
        ...
        if self.curr_bucket_key in self._buckets:
            if data_shapes is not None:
                if tuple(data_shapes.items()) not in self._buckets[self.curr_bucket_key]['exe']:
                    ...
                    self._buckets[self.curr_bucket_key]['data_shapes'] = data_shapes
            return
        # 2. If the bucket key does not exist, create hunterdqn-v3-single-centralized symbol + executor
        assert data_shapes is not None, "Must set data_shapes for centralized_dqn bucket!"
