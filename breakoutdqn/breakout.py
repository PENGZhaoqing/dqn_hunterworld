...
from breakoutdqn.BrainDQN_mx import sym, createQNetwork, copyTargetQNetwork
from breakoutdqn.replay_memory import ReplayMemory
import gym
import cv2
import mxnet as mx
...
def main():
    ...
    if testing:
        ...
        eps_start = 0.11
        param_file = "saved_networks/network-dqn_mx0010.params"
        epoch_start = 19

    if training_continue:
        param_file = "saved_networks_agent2/network-dqn_mx007.params"
        epoch_start = 19
        eps_start = 0.1
    ...
    # action_num = 6
    action_matrix = np.zeros((action_num, action_num))
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        while steps_left > 0:
            ...
            while not terminal_flag:
                ...
                next_ob, reward, terminal_flag, _info = env.step(action)
                gray_image = cv2.cvtColor(next_ob, cv2.COLOR_BGR2GRAY)
                next_ob = cv2.resize(gray_image, (84, 84), interpolation=cv2.INTER_LINEAR)
        ...
        epoch_str = ("Epoch:%d, FPS:%f, Avg Reward: %f/%d"
