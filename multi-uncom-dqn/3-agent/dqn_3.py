...
from ANN_3 import createQNetwork, copyTargetQNetwork
...
def main():
    ...
    if testing:
        ...
        param_file3a = "saved_networks_agent3/network-dqn_mx0019.params"

    if not testing:
        logging_config(folder_name="log", name="non_com_network_a3")
    ...
    replay_memory3 = ReplayMemory(state_dim=(74,),
                                  history_length=history_length,
                                  memory_size=replay_memory_size,
                                  replay_start_size=replay_start_size, state_dtype='float16')
    ...
    a2_target1 = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1)
    a2_target32 = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32)
    a2_Qnet = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32)

    a3_target1 = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1)
    a3_target32 = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32)
    a3_Qnet = createQNetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32)
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        while steps_left > 0:
            ...
            while not env.game_over():
                if replay_memory1.size >= history_length and replay_memory1.size > replay_start_size:
                    ...
                    if do_exploration:
                        ...
                        episode_q_value += q_value3[action3]
                ...
                replay_memory2.append(next_ob[1].flatten(), action2, reward[1], terminal_flag)
                replay_memory3.append(next_ob[2].flatten(), action3, reward[2], terminal_flag)
                ...
                if total_steps % update_interval == 0 and replay_memory1.size > replay_start_size:
                    ...
                    a3_Qnet.forward(mx.io.DataBatch([state_batch3, actions_batch3, y_batch3],
                                                    []), is_train=True)
                    a3_Qnet.backward()
                    a3_Qnet.update()
            ...
            info_str = "Epoch:%d, Episode:%d, Steps Left:%d/%d/%d, Score:%f, fps:%f, Exploration:%f" \
