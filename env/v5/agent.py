...
class Prey(Agent):
    ...
    def update(self, dt):
        ...
        if self.pos[0] + dx > self.SCREEN_WIDTH - self.radius:
            self.pos[0] = self.SCREEN_WIDTH - self.radius
            self.direction[0] = - self.direction[0]  # a little jitter
        elif self.pos[0] + dx <= self.radius:
            self.pos[0] = self.radius
            self.direction[0] = - self.direction[0]  # a little jitter
        else:
            self.pos[0] += dx

        if self.pos[1] + dy > self.SCREEN_HEIGHT - self.radius:
            self.pos[1] = self.SCREEN_HEIGHT - self.radius
            self.direction[1] = - self.direction[1]
        elif self.pos[1] + dy <= self.radius:
            self.pos[1] = self.radius
            self.direction[1] = - self.direction[1]  # a little jitter
        else:
            self.pos[1] += dy
