...
class HunterWorld(PyGameWrapper):
    ...
    def draw_line(self, hunter, observation):
        ...
        observation = (1 - observation) * hunter.out_radius
        for i in range(0, self.EYES):
            ...
            if line > radius:
                ...
                end_pos = Vec2d((0, 0))
                end_pos[0] = center[0] + int(sin_angle * line)
                end_pos[1] = center[1] - int(cos_angle * line)
    ...
    def observe1(self, hunter, others):
        ...
        observation = np.zeros((self.EYES, 9), dtype=np.float32)
        angle = 2 * np.pi / self.EYES
        other_agents = others[:]
        for i in range(0, self.EYES):
            ...
            for agent in other_agents:
                ...
                if dis is not False:
                    if type(agent) is Prey:
                        observation[i] = [1.0 - dis / out_radius, agent.velocity[0], agent.velocity[1], 0, 0, 0, 0, 0,
                                          0]
                    elif type(agent) is Hunter:
                        observation[i] = [0, 0, 0, 1.0 - dis / out_radius, agent.velocity[0], agent.velocity[1], 0, 0,
                                          0]
                    elif type(agent) is Toxin:
                        observation[i] = [0, 0, 0, 0, 0, 0, 1.0 - dis / out_radius, agent.velocity[0],
                                          agent.velocity[1]]
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = HunterWorld(width=258, height=258, num_preys=10, num_hunters=2,num_toxins=10)
    ...
    while True:
        ...
        if game.game_over():
            game.init()
        reward = game.step(dt, False, True)
        # print game.lives
