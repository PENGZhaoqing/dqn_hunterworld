...
class HunterWorld(PyGameWrapper):
    ...
    def _fix_start(self):
        pos_list = [[0.45, 0.45], [0.65, 0.65], [0.35, 0.75],
    ...
    def step(self, dt, frame_skip, draw):
        ...
        if frame_skip is False:
            ...
            for hunter in self.hunters:
                ...
                state = np.append(ob,
                                  np.array(
                                      ((hunter.velocity[0] + self.width) / (self.width * 2),
                                       (hunter.velocity[1] + self.width) / (self.height * 2)),
                                      dtype=np.float16))
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = HunterWorld(width=258, height=258, num_preys=10, num_hunters=3)
