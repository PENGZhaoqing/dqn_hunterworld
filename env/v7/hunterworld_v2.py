...
from env.v7.agent import Hunter, Prey, Toxin
...
class HunterWorld(PyGameWrapper):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                for idx, actions in self.actions.iteritems():
                    ...
                    if key == actions["left"]:
                        hunters[idx].direction[0] = -hunters[idx].speed
                        hunters[idx].direction[1] = 0
                    ...
                    if key == actions["up"]:
                        hunters[idx].direction[1] = -hunters[idx].speed
                        hunters[idx].direction[0] = 0
    ...
    def draw_line(self, hunter, observation):
        ...
        for i in range(1, self.EYES + 1):
            ...
            index = np.argmin(observation[i - 1])
            line = observation[i - 1][index]
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = HunterWorld(width=500, height=500, num_preys=10, num_hunters=2)
